          ...
          <v-col cols="12">
            <v-data-table :headers="headers" :items="suggestions" class="elevation-0 rounded-2xl">
              ...
              <template #item.date_created="{ item }">
                <div>{{ formatDate(item.date_created, 'DD-MM-YYYY') }}</div>
              </template>
...
import type { AdminSuggestions, Suggestions } from '@/types/general'
import dayjs from 'dayjs'

const router = useRouter()
const authStore = useAuthStore()
const suggestions = ref<AdminSuggestions[]>([])
...
const selectedItem = ref<AdminSuggestions>()
...
const openStatusDialog = (item: AdminSuggestions) => {
...
const saveStatus = async () => {
  ...
  const response = await adminApi.UpdateSuggestionStatus(
  ...
  selectedItem.value.status = selectedStatus.value
...
onMounted(async () => {
  try {
    const response = await adminApi.GetAllSuggestions(authStore.accessToken)
    if (response.data.success) {
...
const headers = [
  ...
  { title: 'Date', key: 'date_created' },
  { title: 'Created By', key: 'employee' },
...
const ViewSuggestion = async (item: Suggestions) => {
  ...
  preview.value = []
  try {
    const response = await adminApi.GetSuggestionById(authStore.accessToken, Number(item.id))

    if (!response.data.success) {
      console.log(response.data.message)
      return
    }
    const result = response.data.suggestion
